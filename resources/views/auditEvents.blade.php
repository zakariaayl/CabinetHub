...
@section('title', 'Event Audit Logs')
    ...
    <div class="mb-8">
        <form method="GET" action="" class="max-w-2xl mx-auto">
            ...
            <div class="relative">
                ...
                <button type="submit" class="absolute right-3 top-1/2 transform -translate-y-1/2 px-4 py-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl hover:from-blue-600 hover:to-indigo-700 transition-all duration-300">
                    Search
                </button>
            </div>
        </form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    filterToggle.addEventListener('click', function() {
        ...
        filtersPanel.classList.toggle('animate-pulse');
    });
});
...
function showAuditDetail(auditId) {
    ...
    fetch(`/event-audits/${auditId}`)
        .then(response => response.json())
        .then(data => {
            modalContent.innerHTML = `
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-4">
                        <div class="bg-gradient-to-r from-slate-50 to-slate-100 p-4 rounded-2xl">
                            <label class="text-sm font-semibold text-slate-600">Event Type</label>
                            <div class="mt-2">
                                <span class="inline-flex items-center px-4 py-2 rounded-xl text-sm font-medium bg-gradient-to-r ${getEventTypeGradient(data.event_type.toLowerCase())} text-white shadow-sm">
                                    <i class="fas ${getEventTypeIcon(data.event_type.toLowerCase())} mr-2"></i>
                                    ${data.event_type}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="bg-gradient-to-r from-slate-50 to-slate-100 p-4 rounded-2xl">
                            <label class="text-sm font-semibold text-slate-600">Model Information</label>
                            <div class="mt-2 text-slate-800">
                                ${data.model_type ? `${data.model_type}${data.model_id ? ` #${data.model_id}` : ''}` : 'N/A'}
                            </div>
                        </div>

                        <div class="bg-gradient-to-r from-slate-50 to-slate-100 p-4 rounded-2xl">
                            <label class="text-sm font-semibold text-slate-600">User</label>
                            <div class="mt-2 text-slate-800">
                                ${data.user ? `${data.user.name} (#${data.user.id})` : (data.user_id ? `User #${data.user_id}` : 'System')}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div class="bg-gradient-to-r from-slate-50 to-slate-100 p-4 rounded-2xl">
                            <label class="text-sm font-semibold text-slate-600">Date & Time</label>
                            <div class="mt-2 text-slate-800">
                                ${data.date_event} at ${data.created_at}
                            </div>
                        </div>

                        <div class="bg-gradient-to-r from-slate-50 to-slate-100 p-4 rounded-2xl">
                            <label class="text-sm font-semibold text-slate-600">IP Address</label>
                            <div class="mt-2">
                                <code class="px-3 py-1 bg-slate-200 rounded-lg text-sm">${data.ip_address || 'N/A'}</code>
                            </div>
                        </div>

                        <div class="bg-gradient-to-r from-slate-50 to-slate-100 p-4 rounded-2xl">
                            <label class="text-sm font-semibold text-slate-600">Event ID</label>
                            <div class="mt-2 text-slate-800">
                                #${data.id}
                            </div>
                        </div>
                ...
                <div class="mt-6 bg-gradient-to-r from-slate-50 to-slate-100 p-4 rounded-2xl">
                    <label class="text-sm font-semibold text-slate-600">Description</label>
                    <div class="mt-2 p-4 bg-white rounded-xl border border-slate-200 text-slate-800">
                        ${data.description || 'No description available'}
                    </div>
                </div>

                ${data.old_values ? `
                <div class="mt-6 bg-gradient-to-r from-red-50 to-pink-50 p-4 rounded-2xl border border-red-200">
                    <label class="text-sm font-semibold text-red-700">Previous Values</label>
                    <pre class="mt-2 p-4 bg-white rounded-xl text-sm overflow-x-auto border border-red-200"><code>${JSON.stringify(data.old_values, null, 2)}</code></pre>
                </div>
                ` : ''}

                ${data.new_values ? `
                <div class="mt-6 bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-2xl border border-green-200">
                    <label class="text-sm font-semibold text-green-700">New Values</label>
                    <pre class="mt-2 p-4 bg-white rounded-xl text-sm overflow-x-auto border border-green-200"><code>${JSON.stringify(data.new_values, null, 2)}</code></pre>
                </div>
                ` : ''}
            `;
        })
        .catch(error => {
            modalContent.innerHTML = `
                <div class="text-center py-8">
                    ...
                    <h3 class="text-lg font-semibold text-slate-800 mb-2">Error Loading Details</h3>
                    <p class="text-slate-600">Unable to load audit details. Please try again.</p>
                </div>
            `;
        });
}
